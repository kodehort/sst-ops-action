...
# LOCAL EDIT (re-apply after any `fallow hooks install`): this repo is managed
# with jj, not git, so the stock git-only tokenizer above never fires and the
# gate is a silent no-op. Mirrors is_git_write_command for the jj verbs that
# create or amend a commit. `split` is not optional — the jj-commit skill
# creates most commits that way, and omitting it leaves the normal flow ungated.
is_jj_write_command() {
  local cmd="$1" segment
  while IFS= read -r segment; do
    # Intentional word splitting; globbing is disabled at the call site.
    # shellcheck disable=SC2086
    set -- $segment
    while [ "$#" -gt 0 ]; do
      if [ "$1" != "jj" ]; then
      ...
      while [ "$#" -gt 0 ]; do
        case "$1" in
          # `ci` and `desc` are jj's own default aliases for commit/describe.
          # `squash` amends the parent commit, so it lands work too.
          # (`jj git push` is already caught by is_git_write_command, which
          # scans for the `git` token anywhere in the segment.)
          commit | ci | describe | desc | split | squash)
            return 0
            ;;
          -R | --repository | --at-operation | --at-op | --config | --config-file | --config-toml | --color)
            # Global option whose value arrives as the next word.
            shift
            [ "$#" -gt 0 ] && shift
            ;;
          -*)
            ...
            ;;
          *)
            # A different subcommand; resume scanning for a later `jj` word.
            break
            ;;
        esac
      done
...
set -f
if is_git_write_command "$CMD" || is_jj_write_command "$CMD"; then
  GIT_WRITE=1
else
  GIT_WRITE=0
fi
...
if [ "$GIT_WRITE" -eq 0 ]; then
  if [ -n "${FALLOW_GATE_DEBUG:-}" ]; then
    echo "fallow-gate: not a git/jj commit or push, skipping audit." >&2
  fi
  exit 0
fi

# LOCAL EDIT (re-apply after any `fallow hooks install`): fallow is a pinned
# devDependency here, not a global binary, and this repo uses bun. Probed first
# so the pinned version wins over any stale global install; without it the
# stock PATH/npx precedence finds nothing and the gate skips silently.
if command -v bun >/dev/null 2>&1 && VER_PROBE="$(bun fallow --version 2>/dev/null || true)" && [[ "$VER_PROBE" == fallow* ]]; then
  RUNNER=(bun fallow)
  BIN_DESC="bun fallow"
