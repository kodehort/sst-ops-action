...
// Test constants
const TEST_TIMEOUT = 30000; // 30 seconds for real CLI operations
...
describe('Real SST CLI Integration Tests', () => {
  ...
  describe('SST CLI Availability', () => {
    it('should detect if SST CLI is available', () => {
      ...
      if (sstAvailable) {
        console.log('✅ SST CLI is available for integration tests');
      } else {
        console.log('⚠️ SST CLI is not available - integration tests will be skipped');
      }
  ...
  describe('SST Diff Operation', () => {
    it('should execute sst diff command successfully', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST diff test - CLI not available');
        return;
      }
    ...
    it('should handle diff with invalid stage gracefully', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST diff invalid stage test - CLI not available');
        return;
      }
      ...
      const result = await executeSSTCommand(
        ...
        testProjectPath,
        10000 // Shorter timeout for error case
      );
  ...
  describe('SST Deploy Operation (Dry Run)', () => {
    it('should validate deployment configuration without actually deploying', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST deploy validation test - CLI not available');
        return;
      }
  ...
  describe('SST Remove Operation (Safe)', () => {
    it('should handle remove command on non-existent stage gracefully', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST remove test - CLI not available');
        return;
      }
  ...
  describe('SST Error Scenarios', () => {
    it('should handle malformed sst.config.ts file', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST malformed config test - CLI not available');
        return;
      }

      // Overwrite with malformed config
      writeFileSync(join(testProjectPath, 'sst.config.ts'), 'invalid typescript syntax{{{');

      const result = await executeSSTCommand(['diff'], testProjectPath, 10000);
    ...
    it('should handle missing sst.config.ts file', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST missing config test - CLI not available');
        return;
      }
  ...
  describe('Environment Integration', () => {
    it('should respect SST_STAGE environment variable', async () => {
      if (!sstAvailable) {
        console.log('Skipping SST environment test - CLI not available');
        return;
      }

      const customStage = 'custom-test-stage';
      const result = await executeSSTCommand(['diff'], testProjectPath);
